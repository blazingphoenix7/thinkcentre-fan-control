...
displays these values we measured how each byte behaves from idle through a
...
We have no verified mapping from these offsets to physical components. Several
...
1. **No per-component labels.** The UI never says "CPU", "GPU", etc. It says
   **"hottest sensor"**, which is exactly what we can defend.
...
and turn these into named sensors. Until then, honest and unlabeled beats
confident and wrong. `0x26` in particular deserves a decode attempt (fixed
offset from 0x23? different unit? tjunction-relative?) before it is ever shown.
